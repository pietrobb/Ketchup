        ...
        async with self.lock:
            ...
            try:
                ...
                if isinstance(error, Rejection):
                    ...
                    value = _error(code, _REJECTION_HINTS.get(code, "Live bridge rejected the request."),
...
def _register_tools(plan_state, *, launcher=None, discoverer=None, attacher=None) -> list:
    ...
    async def model(action: str, handle: str, expected: dict | None = None,
        ...
        One user request = one apply_and_verify; its collision check IS the verification (no tests, no images).
