...
const STRENGTHENED_FREEZE_ID: &str = "strengthened-a0-v2-r2";
const SEEDS: [u64; 20] = [
...
fn gate_a0_v2() {
    ...
    run_guaranteed_mutations(&backend, &mut metrics);
    ...
    write_evidence(&metrics, &failures);
    assert!(
        failures.is_empty(),
        "Strengthened A0 v2 inherited suite NO-GO: {}",
        failures.join("; ")
    );
}
...
fn validate_active_freeze(repository: &Path) {
    ...
    let run_suffix = run_id
        .strip_prefix("strengthened-v2-r2-run-")
        .expect("run ID must use the strengthened-v2-r2-run-NNN namespace");
    assert!(
...
fn run_guaranteed_mutations(backend: &ExactBackend, metrics: &mut Metrics) {
    let base = backend
        .extrude_rectangle(RectangleExtrudeSpec {
            width_mm: 100.0,
            depth_mm: 60.0,
...
fn write_evidence(metrics: &Metrics, failures: &[String]) {
    let path = PathBuf::from(
        std::env::var_os("KETCHUP_A0_V2_METRICS_PATH")
            .expect("runner must provide a unique inherited-suite metrics path"),
    );
    assert!(
        !path.exists(),
        "A0 v2 inherited-suite evidence path already exists"
    );
    let lock_sha256 = std::env::var("KETCHUP_A0_V2_LOCK_SHA256")
        .expect("runner must provide the validated v2 lock hash");
    ...
    fs::write(path, raw).expect("immutable A0 v2 inherited-suite evidence must be written");
}
