[CmdletBinding()]
param([string]$RunId = "strengthened-v2-r2-run-001")
...
$preregistrationPath = Join-Path $artifactRoot "strengthened-a0-v2-r2-preregistration.json"
$lockPath = Join-Path $artifactRoot "strengthened-a0-v2-r2-lock.json"
    ...
    try {
        ...
        try {
            ...
            if ($null -ne $process) {
                ...
                if ($NativeObservation -and $jobAssigned -and $null -ne $job -and -not $job.WaitForEmpty(10000)) { $script:cleanupBarrierPassed = $false }
...
if ($RunId -notmatch '^strengthened-v2-r2-run-[0-9]{3}$') { throw "RunId must match strengthened-v2-r2-run-NNN." }
...
try {
    ...
    $preflightRecord = Invoke-CapturedProcess `
        ...
        @{} `
        ([ordered]@{ freeze_id = "strengthened-a0-v2-r2" }) `
        $false `
        $null `
        180
