...
$lockPath = Join-Path $repoRoot "artifacts\gate-a0\strengthened-a0-v2-lock.json"
$preregistrationPath = Join-Path $repoRoot "artifacts\gate-a0\strengthened-a0-v2-preregistration.json"
...
if ($lock.freeze_id -ne "strengthened-a0-v2" -or $lock.measurement_state_at_freeze -ne "not_started") {
    throw "Unexpected strengthened A0 v2 freeze identity or measurement state."
}
if ($preregistration.gate_id -ne "strengthened-a0-v2" -or
...
$requiredFiles = @(
    "artifacts/gate-a0/strengthened-a0-v2-preregistration.json",
