    ...
    for reference in &references {
        match resolve_subshape_reference(reference, &output) {
            ReferenceResolution::Resolved {
                ...
                assert_eq!(
                    migrated_backend, migrated,
                    "diagnostic consumer migration flag disagrees with build identity"
                );
                let face = output
                    .body
                    .topology
                ...
                assert!(
                    ...
                    "diagnostic consumer resolved the wrong semantic face for {}",
                    reference.semantic_role
                );
                assert_eq!(
                    face.edge_ordinals.len(),
                    4,
                    "diagnostic consumer resolved face has incomplete boundary"
                );
                println!("resolved={}@{}", reference.semantic_role, face_ordinal);
            }
            other => panic!(
                "diagnostic consumer did not resolve {}: {other:?}",
                reference.semantic_role
            ),
        }
    }
...
fn expected_semantic_face(reference: &SubshapeRef, face: &FaceEvidence) -> bool {
    match reference.semantic_role.as_str() {
        "extrusion.top" => close(face.centroid_mm.z, 25.0),
        "extrusion.bottom" => close(face.centroid_mm.z, 0.0),
        "extrusion.side(profile_edge=east)" => close(face.centroid_mm.x, 120.0),
        _ => false,
    }
}
