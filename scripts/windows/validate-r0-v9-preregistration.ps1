...
$oldLockPath = Join-Path $repoRoot "artifacts\r0\preregistration-lock-r0-v8.json"
$activeLockPath = Join-Path $repoRoot "artifacts\r0\preregistration-lock-r0-v9.json"
$old = Get-Content $oldLockPath -Raw | ConvertFrom-Json
$active = Get-Content $activeLockPath -Raw | ConvertFrom-Json

if ($old.freeze_id -ne "r0-v8" -or $active.freeze_id -ne "r0-v9") {
...
if (($changed -join "|") -ne "Cargo.lock") {
    throw "r0-v9 may differ from r0-v8 only in Cargo.lock; observed: $($changed -join ', ')"
}
...
$schedulerManifest = Get-Content (Join-Path $repoRoot "crates\ketchup-scheduler\Cargo.toml") -Raw
if (-not $schedulerManifest.Contains('ketchup-interaction = { path = "../ketchup-interaction" }')) {
    throw "The Gate C core runner local interaction edge is missing."
}
...
$report = Get-Content (Join-Path $repoRoot "docs\gates\R0_V9_REPORT.md") -Raw
...
if (-not $report.Contains("**Decision: GO**") -or -not $report.Contains($activeHash)) {
    throw "R0 v9 report does not authorize this exact lock."
}

Write-Output "R0 v9 validation passed: only the local Gate C runner interaction edge changed Cargo.lock; all frozen thresholds, corpora, licenses, OCCT inputs, hardware profiles, and consequences remain byte-identical to r0-v8."
