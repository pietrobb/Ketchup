...
$oldLockPath = Join-Path $repoRoot "artifacts\r0\preregistration-lock-r0-v3.json"
$activeLockPath = Join-Path $repoRoot "artifacts\r0\preregistration-lock-r0-v4.json"
$old = Get-Content $oldLockPath -Raw | ConvertFrom-Json
$active = Get-Content $activeLockPath -Raw | ConvertFrom-Json

if ($old.freeze_id -ne "r0-v3" -or $active.freeze_id -ne "r0-v4") {
...
if (($changed -join "|") -ne ".gitattributes") {
    throw "r0-v4 may differ from r0-v3 only in .gitattributes; observed: $($changed -join ', ')"
}
...
$manifest = [Array]::IndexOf($attributeLines, "artifacts/r0/occt-build-manifest.json -text")
$config = [Array]::IndexOf($attributeLines, "artifacts/r0/occt-cmake-config.json -text")
...
$report = Get-Content (Join-Path $repoRoot "docs\gates\R0_V4_REPORT.md") -Raw
...
if (-not $report.Contains("**Decision: GO**") -or -not $report.Contains($activeHash)) {
    throw "R0 v4 report does not authorize this exact lock."
}

Write-Output "R0 v4 validation passed: staged-byte preservation is effective and all frozen thresholds, corpora, dependencies, and toolchain evidence remain byte-identical to r0-v3."
