...
function Invoke-CapturedProcess(
    ...
    try {
        if ($NativeObservation) {
            ...
            $missingSourceDlls = @($sourceDlls | Where-Object { -not (Test-Path (Join-Path $RuntimeRoot $_.Name) -PathType Leaf) })
            if ($sourceDlls.Count -eq 0 -or $missingSourceDlls.Count -ne 0) {
                throw "Staged runtime is missing one or more frozen backend DLLs."
            }
...
try {
    ...
    foreach ($combination in $combinations) {
        ...
        $consumerStdout = [string](Get-Content (Join-Path $runDir $consumerRecord.stdout_path) -Raw)
