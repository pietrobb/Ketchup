...
$lockPath = Join-Path $repoRoot "artifacts\gate-a0\strengthened-a0-v2-r3-lock.json"
$preregistrationPath = Join-Path $repoRoot "artifacts\gate-a0\strengthened-a0-v2-r3-preregistration.json"
...
if ($lock.freeze_id -ne "strengthened-a0-v2-r3" -or $lock.measurement_state_at_freeze -ne "not_started") {
    throw "Unexpected strengthened A0 v2 freeze identity or measurement state."
}
if ($preregistration.gate_id -ne "strengthened-a0-v2-r3" -or
...
$r2SealPath = Join-Path $repoRoot "artifacts\gate-a0\runs\strengthened-v2-r2-run-001\seal.json"
$r2Root = Split-Path $r2SealPath -Parent
$r2Seal = Get-Content $r2SealPath -Raw | ConvertFrom-Json
foreach ($entry in @($r2Seal.files)) {
    $sealedPath = [IO.Path]::GetFullPath((Join-Path $r2Root ([string]$entry.path)))
    if (-not $sealedPath.StartsWith($r2Root.TrimEnd("\") + "\", [StringComparison]::OrdinalIgnoreCase) -or
        -not (Test-Path $sealedPath -PathType Leaf) -or
        (Get-FileHash $sealedPath -Algorithm SHA256).Hash.ToLowerInvariant() -ne [string]$entry.sha256) {
        throw "A0 v2 r2 sealed evidence changed or escaped its run directory: $($entry.path)"
    }
}

$requiredFiles = @(
    "artifacts/gate-a0/strengthened-a0-v2-r3-preregistration.json",
    "artifacts/gate-a0/strengthened-a0-v2-lock.json",
    "artifacts/gate-a0/runs/strengthened-v2-run-001/seal.json",
    "artifacts/gate-a0/strengthened-a0-v2-r2-lock.json",
    "artifacts/gate-a0/runs/strengthened-v2-r2-run-001/seal.json",
