...
$oldLockPath = Join-Path $repoRoot "artifacts\r0\preregistration-lock-r0-v6.json"
$activeLockPath = Join-Path $repoRoot "artifacts\r0\preregistration-lock-r0-v7.json"
$old = Get-Content $oldLockPath -Raw | ConvertFrom-Json
$active = Get-Content $activeLockPath -Raw | ConvertFrom-Json

if ($old.freeze_id -ne "r0-v6" -or $active.freeze_id -ne "r0-v7") {
...
if (($changed -join "|") -ne "Cargo.lock|deny.toml") {
    throw "r0-v7 may differ from r0-v6 only in Cargo.lock and deny.toml; observed: $($changed -join ', ')"
}
...
$denyPolicy = Get-Content (Join-Path $repoRoot "deny.toml") -Raw
foreach ($license in @("BSL-1.0", "CC0-1.0", "OFL-1.1", "Ubuntu-font-1.0")) {
    if (-not $denyPolicy.Contains("`"$license`"")) { throw "Gate C license is not explicitly allowed: $license" }
}
...
$report = Get-Content (Join-Path $repoRoot "docs\gates\R0_V7_REPORT.md") -Raw
...
if (-not $report.Contains("**Decision: GO**") -or -not $report.Contains($activeHash)) {
    throw "R0 v7 report does not authorize this exact lock."
}

Write-Output "R0 v7 validation passed: Gate C eframe/egui/wgpu dependencies and four permissive/free licenses are pinned and audited; all frozen thresholds, corpora, OCCT inputs, hardware profiles, and consequences remain byte-identical to r0-v6."
