...
if ($runnerSource -match 'Type READY only while the requested native file dialog remains open' -or
    ...
    $runnerSource -notmatch [regex]::Escape('$dialogObservationTimeout = [TimeSpan]::FromMinutes(2)') -or
...
try {
    ...
    $unboundedDialogWaitEvidence.native_file_dialogs[0].observation_wait_elapsed_ms = 120001
    Write-Utf8 (Join-Path $tempRoot "evidence-manifest.json") (($unboundedDialogWaitEvidence | ConvertTo-Json -Depth 12) + "`n")
    Invoke-VerifyExpectingFailure "a native-dialog poll presented after exceeding its monotonic two-minute wait" "Native file-dialog observation was not bounded by its monotonic two-minute wait"
    Write-Manifest $false
