...
def test_cad_append_sweep_matches_rust_boundaries_and_strict_fields():
    ...
    assert "closed polygon or line/arc profile" in assistant.SYSTEM_PROMPT

    invalid_features = [
        {**feature, "profile_feature_id": 0},
        {**feature, "profile_feature_id": True},
        {**feature, "profile_feature_id": assistant.MAX_U64 + 1},
        {**feature, "path_feature_id": 11},
        {**feature, "path_feature_id": 0},
        {**feature, "path_feature_id": True},
        {"type": "sweep", "profile_feature_id": 11},
        {**feature, "path_feature_id": assistant.MAX_U64 + 1},
        {**feature, "output_feature_id": 99},
    ]
    for invalid_feature in invalid_features:
        with pytest.raises(assistant.ProtocolError):
...
def test_cad_append_topology_shell_matches_rust_boundaries_and_strict_fields():
    reference_id = "a" * 64
    feature = {
        "type": "topology_shell",
        "target_feature_id": 11,
    ...
    assert "Never invent topology reference IDs" in assistant.SYSTEM_PROMPT
    assistant._validate_cad_edit_program(
        {
            "operations": [
                {
    ...
    invalid_features = [
        ...
        {**feature, "target_feature_id": True},
        {**feature, "target_feature_id": assistant.MAX_U64 + 1},
        {**feature, "removed_face_reference_ids": []},
        {**feature, "removed_face_reference_ids": [reference_id] * 2},
        {**feature, "removed_face_reference_ids": ["short"]},
        {**feature, "removed_face_reference_ids": ["z" * 64]},
        {**feature, "removed_face_reference_ids": [reference_id] * 65},
        {**feature, "thickness_mm": 0.009},
        {**feature, "thickness_mm": 100_001},
        {**feature, "thickness_mm": float("nan")},
        {**feature, "thickness_mm": True},
        {**feature, "face_ordinal": 3},
        {"type": "topology_shell", "target_feature_id": 11, "thickness_mm": 1.5},
    ]
    for invalid_feature in invalid_features:
        with pytest.raises(assistant.ProtocolError):
            assistant._validate_cad_edit_program(
...
def test_cad_append_topology_chamfer_matches_rust_boundaries_and_strict_fields():
    reference_id = "b" * 64
    feature = {
    ...
    assert "type: topology_chamfer" in assistant.SYSTEM_PROMPT
    assistant._validate_cad_edit_program(
        {
            "operations": [
                {
    ...
    invalid_features = [
        ...
        {**feature, "distance_mm": 100_001},
        {**feature, "distance_mm": float("nan")},
        {**feature, "distance_mm": True},
        {**feature, "edge_ordinal": 3},
        {**feature, "kind": "fillet"},
        {"type": "topology_chamfer", "target_feature_id": 11, "distance_mm": 1.5},
    ]
    for invalid_feature in invalid_features:
        with pytest.raises(assistant.ProtocolError):
            assistant._validate_cad_edit_program(
...
def test_public_sidecar_parses_only_bounded_smooth_teapot_workflows():
    ...
    assert "nest teapot inside that same item" in assistant.SYSTEM_PROMPT
...
def test_public_sidecar_parses_bounded_ketchup_squeeze_bottle():
    ...
    assert "nest ketchup_bottle inside that same item" in assistant.SYSTEM_PROMPT
    assert "cap height strictly between neck height plus twice wall thickness" in assistant.SYSTEM_PROMPT
    invalid = json.loads(answer)
