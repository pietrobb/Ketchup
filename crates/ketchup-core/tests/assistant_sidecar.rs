...
fn cad_edit_append_pocket_contract_is_strict_bounded_and_host_id_assigned() {
    ...
    assert_eq!(program.validate(), Ok(()));
    let serialized = serde_json::to_value(&program).unwrap();
    assert_eq!(serialized["operations"][0]["feature"]["type"], "pocket");
    assert!(
        serialized["operations"][0]
            .get("output_feature_id")
            .is_none()
    ...
    assert!(serde_json::from_value::<AssistantCadEditProgram>(injected_id).is_err());
}
...
#[test]
fn cad_edit_append_topology_fillet_contract_is_strict_bounded_and_host_id_assigned() {
    let reference_id = "a".repeat(64);
