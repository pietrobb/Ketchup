...
SYSTEM_PROMPT = (
    ...
    "If the requested target or operation is ambiguous, ask one concrete clarification question and return every action field as null; never guess a target or host ID. "
...
def _validate_cad_edit_program(program: object) -> dict:
    ...
    for operation_index, operation in enumerate(operations):
        ...
        elif operation_type == "make_occurrence_unique":
            if set(operation) != {"operation", "occurrence_id"}:
                raise ProtocolError("provider CAD make-unique contains missing or unknown fields")
            occurrence_id = operation["occurrence_id"]
            if (
                not isinstance(occurrence_id, int)
                or isinstance(occurrence_id, bool)
                or not 0 < occurrence_id <= MAX_U64
            ):
                raise ProtocolError("provider CAD make-unique occurrence is invalid")
        ...
        elif operation_type == "set_grounded":
            if set(operation) != {"operation", "selector", "grounded"} or not isinstance(
                operation["grounded"], bool
            ):
                raise ProtocolError("provider CAD grounding is invalid")
