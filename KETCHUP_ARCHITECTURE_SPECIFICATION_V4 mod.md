...
3. this V4 specification after its review dispositions are accepted;
...
A second bounded organic reference workflow is one editable living tree: trunk and branch structure has semantic paths and variable-radius sweep/loft or explicitly mesh-authoritative procedural geometry. Ketchup’s target is not “every mathematically imaginable shape,” unrestricted sculpting, animation, or production-scale vegetation generation. It is selected exact and mesh workflows that make furniture, buildings, bottles, trees, and similarly understandable shapes quick to create and revise, with honest fallback and loss reporting.
...
Therefore validators, stable derivation identity, dimensions, BOMs, and bounded joint semantics are product features rather than optional internal infrastructure. Core MUST expose a domain-neutral validator protocol and policy boundary so open-source and proprietary/paid packages can add standards, spacing/clearance, structural/statics, manufacturability, and advisory checks without creating a second document authority.
...
Operation-family priority is determined by frozen user-workflow evidence, not by completing this table. The bottle and living-tree workflows in §1.6 are reference tests: each operation must remain directly editable, numerically controllable where meaningful, Undoable, persistable, and explicit about exact versus mesh authority. A bounded individual-tree workflow does not imply a vegetation/world generator.
...
| V4-P14 | **PROPOSED, REVISED AFTER REVIEW** | Validators are first-class deterministic services behind an open domain-neutral protocol. Packages may be open-source or proprietary/paid and may block commit/export according to versioned policy, but licensing never grants mutation authority or weakens determinism, provenance, privacy, budgets, or fail-closed behavior. |
| V4-P15 | **PROPOSED; DEDICATED ADR REQUIRED; REVISED AFTER REVIEW** | Replace the old monolithic Gate C sequence with C1a before additional proxy-modeler stabilization, execute product-validation milestone M4a immediately after M2 and before OCCT product integration, then execute C1b after exact product integration. This is a sequence change, not a clarification. |
...
All `V4-P*` requirements are **proposed MUSTs** until accepted by the accountable project owner. Ratification requires exactly three decision records: (1) one V4 adoption ADR accepting P01–P14 as a coherent baseline; (2) one dedicated P07/P08 ADR governing observational Open, derived-result registration, and canonical-digest authority; and (3) one dedicated P15 ADR governing every deviation from the frozen V3 gate/milestone sequence, including C1a/C1b and M4a-before-M3. The adoption ADR incorporates the dedicated P07/P08 ADR by reference. Separate ADRs for the remaining individual P decisions are not required unless a later change alters their accepted architectural commitment.
...
| P10, P11, P14 | Validation/domain lead | Project owner | collision/intersection/joint corpus, validator-package contract, and beam 6.3a | before M4a exit | V4 adoption ADR |
...
| P15 | Architecture lead | Project owner | explicit old-C disposition and replacement gate charter, including M4a-before-M3 | before relying on reordered sequence, no later than M0 exit | Dedicated P15 ADR |
...
| Validator package protocol | **PLANNED** | No package host, policy registry, trust/licensing state, or availability semantics. | Open domain-neutral read-only protocol for open and proprietary/paid validators with provenance, privacy, budgets, class labels, and fail-closed policy. |
...
- Domain packages MUST extend semantic schemas and validators over the common document; they MUST NOT establish another model database. Their validator protocol and policy contract are open and domain-neutral even when a package implementation is proprietary/paid.
...
`ValidationPolicyRef` resolves a versioned canonical policy containing required/optional validator package identities and versions, accepted publisher/trust requirements, governed standards/jurisdictions/editions, severity and blocking rules, and permitted input/data-egress classes. It stores no license secret and grants no executable capability by itself.
...
- required/optional validator package and policy identities, versions, trust requirements, governing standards/jurisdictions/editions, and last accepted result provenance—never license secrets;
...
Validators are deterministic read-only consumers of a named snapshot and accepted derived-result set. Core owns an open, domain-neutral validator protocol, policy model, diagnostic schema, and capability boundary; it need not contain every domain implementation. Validator packages MAY be open-source or proprietary/paid. Licensing or payment status MUST NOT grant mutation authority or weaken determinism, privacy, resource, provenance, audit, or fail-closed rules.

Every validator package declares stable package identity/version, publisher and trust/signature state, required read scopes, determinism and input envelope, resource limits, policy severity, diagnostic schema, and—where applicable—the governing standard, jurisdiction, and edition. A remote validator additionally declares destination, data classes, retention assumptions, and cancellation/revocation behavior before any egress. Results bind the document revision, accepted derived-result identities, package/policy identity, and complete input digest.
...
SAT is insufficient to classify a declared-joint overlap because it does not locate the complete penetration region. For allowed joint volume `J` and tolerance threshold `ε`, every non-empty penetrating `I_ij` MUST be wholly contained in `J ⊕ ε`; any portion outside is an error. Absence of the required intersection/contact is an empty-joint error. A joint MUST NOT exempt an entire pair of pieces from collision checking. The bounded containment test is what makes the exception safe.
...
3. generate the expected dimension chain, including `415 × 6`, `408 × 5`, `400` for the reference fixture;
...
9. regenerate BOM and dimension outputs;
10. commit the accepted change as one user operation.

This workflow intentionally excludes AI and BTLx so that it proves the deterministic product substrate.
...
WASM Component Model remains a hypothesis, not a binding implementation choice. The validator package contract is required earlier in M4a and is narrower than a general executable plugin marketplace: it exposes only declared snapshot/result reads and structured diagnostics. A paid package uses the same least-privilege host boundary as an open package. Untrusted native or remote validators require process isolation, authenticated package provenance, and the same egress confirmation rules as other external services.
...
- validator package identity/trust/licensing/availability/policy behavior, including required `Unavailable`/`NotEvaluated` fail-closed cases;
- SAT-positive convex-intersection construction and complete allowed-joint-volume containment without double-counted coverage volume.
...
- beam 6.3a immediately after M2/M4a and beam 6.3b only after M3 exact integration;
...
- one directly editable living-tree slice with stable trunk/branch paths and explicit exact/mesh authority;
- representative open and proprietary/paid validator package fixtures with identical capability, provenance, privacy, and fail-closed rules;
...
The order `M0 → durable M1 prerequisites → M2 → M4a → M3 → M4b/M5 → workflow-led M6 → M7` changes the frozen V3 sequence, which expected an exact Gate C before the narrow manual modeler. It remains **proposed** until the dedicated V4-P15 ADR explicitly retires or narrows the old Gate C claim, preserves applicable failure consequences, classifies the change under anti-loosening rules, and gives C1a/C1b/M4a owners, entry criteria, thresholds, and deadlines.

Until that ADR is accepted, the execution contract’s original sequence remains binding. Completed prototype work may be recorded as evidence but cannot be used to claim sequence compliance or retire exact Gate C. The architectural reason for M4a-before-M3 is informational: beam workflow 6.3a depends on M2 rules, hierarchical slots, collision, joints, BOM, and dimensions, but intentionally excludes exact notches, product OCCT integration, durable subshape references, and C1b.
...
**Goal:** prove that the M2 evaluator and identity substrate serves the real fabrication product before investing in product OCCT integration.

1. versioned collision/contact tolerance policy;
2. AABB/OBB/SAT over canonical prismatic convex coverage;
3. deterministic convex-intersection construction and containment within tolerance-inflated joint volumes;
4. canonical bounded joints;
5. hierarchical `SlotPath` overrides and validation diagnostics;
6. open domain-neutral validator package/policy contract, including `Unavailable`/`NotEvaluated` behavior for required packages;
7. BOM and stable entity/`SlotPath` dimension projections that do not require exact subshapes;
8. FurniGen regression import where available;
9. beam workflow 6.3a.

M4a MUST NOT depend on integrated OCCT results, durable `SubshapeRef`, or C1b. If 6.3a reveals that evaluator, slot, override, or policy semantics do not serve the product, M2 is corrected before M3 begins.

**Exit:** one governing value changes the beam output, expected dimensions and joints remain stable or explicitly unresolved, intersection containment catches illegal overlap outside joint volumes, empty declared joints fail, and required unavailable validators cannot masquerade as passing.
...
4. exact-dependent standards, structural, and manufacturability packages selected by frozen domain workflows.
...
2. **Living-tree slice:** semantic trunk/branch `SlotPath` structure; variable-radius sweep/loft or explicitly mesh-authoritative procedural geometry; direct branch/path/radius edits with local recomputation and explicit reference health.
...
6. later domain semantic packages, BIM primitives, and broader drawing/manufacturing integrations; the narrower validator-package protocol already exists from M4a.
...
- Do not begin product OCCT integration before M4a has tested M2 against beam workflow 6.3a, unless the P15 ADR records a new dependency and consequence.
...
| Validator false negatives | Manufacturing, structural, regulatory, or advisory output may look plausible but be invalid or misclassified. | Versioned package identity/policy, convex-intersection/joint/domain corpora, class labels, `NotEvaluated`/`Unavailable`, and fail-closed export policy. |
...
| Validator supply-chain or certification overclaim | Paid/open package status could be mistaken for trust, availability, or legal certification. | Signed/versioned package identity, least privilege, explicit result class and jurisdiction/edition, revocation, no license secrets in documents, and `Unavailable`/`NotEvaluated` fail-closed semantics. |
...
8. Does the collision corpus prove construction of every SAT-positive convex intersection and complete containment in `J ⊕ ε`, without whole-pair exemption or overlap-volume double counting?
9. Do the frozen bottle, living-tree, furniture, and building workflows select the smallest valuable geometry slices without turning M6 into a generic feature-completeness program?
...
- open domain-neutral validator protocol for open-source and proprietary/paid packages;
- exact current gate caveats and CI anti-loosening governance;
- staged migration with strengthened A0 first and M2 → M4a → M3 → M4b/M5 sequencing.

# Appendix D — Review disposition log

This maintained log records every material external-review finding and its disposition. A review is not “incorporated” merely by producing comments; accepted findings must change every affected normative section, while rejected or deferred findings require a stated reason.
...
| R22 | External human architecture reviewer / 2026-08-03 | Beam 6.3a does not depend on M3, so first product measurement was needlessly behind OCCT integration. | **Accept** | P15 and §13 split M4a from exact-dependent M4b and require M2 → M4a → M3. |
...
| R27 | Same | SAT cannot determine whether the penetration region lies inside a bounded joint volume. | **Accept** | P10 and §10 require convex intersection construction and containment in `J ⊕ ε` for every penetrating component pair. |
| R28 | Same | M6 was a general-CAD feature program rather than a sequence selected by valuable workflows. | **Accept** | §§1.6, 2.3, 8.3, 12.4, and M6 define directly editable bottle and living-tree slices and workflow-led operation priority. |
...
| R30 | Project-owner product requirement / 2026-08-03 | Core must support extensible domain validators, including open and paid standards, spacing, structural/statics, manufacturability, and feng-shui packages. | **Accept** | P14 and §§2.2, 6.2, 10.1, 11.5, 12.4, and M4a define an open domain-neutral package/policy protocol with identical security, provenance, licensing, privacy, and fail-closed rules. |

**Review verdict:** `NO-GO for ratification`, retained after three review rounds. The latest material architectural findings have accepted dispositions in this draft. Ratification now requires the three decision records defined by §3.5, a named accountable project owner, disposition of blocking OPEN decisions, and the StateView/fixture/trace evidence package; it no longer requires one ADR per P decision.
...
| V4 body + Appendices A–F prefix bytes | `143574` bytes, ending immediately before Appendix G |
| V4 body + Appendices A–F prefix SHA-256 | `cf178056ad073aef1f1b0eaf40eb31ab6d7241eec931d73fb0ff16ebcb96826f` |
